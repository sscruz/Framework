...
class CutManager:
   ...
   def __init__(self):
      ...
      self.twoLeptonsSynch = "nPairLep_Edge > 0  && Flag_eeBadScFilter_Edge == 1&& Flag_HBHENoiseFilter_Edge ==1 && Flag_HBHENoiseIsoFilter_Edge ==1 && Flag_badChargedHadronFilter_Edge == 1 && Flag_EcalDeadCellTriggerPrimitiveFilter_Edge == 1 && Flag_goodVertices_Edge == 1 && Flag_globalTightHalo2016Filter_Edge ==1 && Flag_CSCTightHalo2016Filter_Edge ==1 "
      ...
      self.goodLeptonNoFilters = "("+self.trigger + "&& nPairLep_Edge > 0 &&" + self.leptonPt + "&&" + self.leptonDR + "&&" + self.leptonsMll +  ")"
      self.goodLepton = "("+self.trigger + "&&" +self.twoLeptonsSynch + "&&" + self.leptonPt + "&&" + self.leptonDR + "&&"  + self.leptonsMll +  ")"
      self.goodLeptonSynch = "("+self.trigger + "&&" +self.twoLeptons + "&&" + self.leptonPt + "&&" + self.leptonDR + "&&"  + self.leptonsMll +  ")"
...
#      self.BaselineNoTrigger = self.AddList([self.nj2,self.METg100,self.dPhiJETMET,self.goodLepton])
      ...
      self.ewinoWZNo3rd      = self.AddList([self.baseline, self.bveto, self.mjj110, self.Zmass])
