...
class CutManager:
   ...
   def __init__(self):
      ...
      ##### Needed for the 3l, 4l and ttZ control regions                                                                                                        
      self.region3l = '('+self.JetMETPhi04 +' && nLepTight_Edge == 3 && met_Edge > 60 && nBJetMedium25_Edge == 0)'
      self.region4l = '('+self.JetMETPhi04 +' && nLepTight_Edge == 4)'
      self.regionttZ ='('+self.JetMETPhi04 +' && nLepTight_Edge == 3 &&  nBJetMedium25_Edge == 2)'                       
      ##### Needed for the fmll and r0b1b calculations for the FS prediction for EWK                                                                                           
      self.ewino =  '('+self.goodLepton+'&&'+self.JetMETPhi04+'&&'+self.SF+'&&'+ self.nj2 +'&&'+ self.Zmass +     ' && nLepLoose_Edge == 2 )'
      self.ewinoClosureReg =  '('+self.goodLepton+'&&'+self.JetMETPhi04+'&&'+ self.nj2 +'&&'+ self.Zmass +        ' && nLepLoose_Edge == 2  && mt2_Edge > 80. && nBJetMedium25_Edge == 0  )'
      self.ewinoClosureExt =  '('+self.goodLepton+'&&'+self.JetMETPhi04+'&&'+ self.nj2 +'&&'+ self.ZmassExtended +' && nLepLoose_Edge == 2  && mt2_Edge > 80.   )'
      self.ewinoCR ='('+self.goodLepton+'&&'+self.JetMETPhi04+'&&'+ self.nj2 +' && nLepLoose_Edge == 2 )'
      self.ewinoSR ='('+self.goodLepton+'&&'+self.JetMETPhi04+'&&'+ self.nj2 +'&&' + self.mjj110 + '&&'+ self.Zmass +' && nBJetMedium25_Edge == 0 && mt2_Edge > 80. && nLepLoose_Edge == 2 && met_Edge > 150 )'
      ...
      self.numerator =   self.AddList([self.goodLepton, self.JetMETPhi04, self.donot(self.JetMETBaseline), self.donot(self.RSFOFDirectControlRegion), self.HT, self.triggerHT, self.trigger])
      self.denominator = self.AddList([self.goodLepton, self.JetMETPhi04, self.donot(self.JetMETBaseline), self.donot(self.RSFOFDirectControlRegion), self.HT, self.triggerHT])  
