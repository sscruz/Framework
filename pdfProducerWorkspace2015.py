...
if __name__ == '__main__':
    ...
    cuts = CutManager.CutManager(True)
    ...
    ## old version deprecated tt_tfile = ROOT.TFile('/afs/cern.ch/work/p/pablom/public/MC/TTLep_pow/treeProducerSusyEdge/tree.root')
    ## old version deprecated #tt_ffile = ROOT.TFile('/afs/cern.ch/work/m/mdunser/public/edgeTrees/friendsForPablosTrees_withSRID/evVarFriend_TTLep_pow.root')
    ## old version deprecated tt_ffile = ROOT.TFile('/afs/cern.ch/work/s/sesanche/public/nTuples/mar02/evVarFriend_TTLep_pow.root')
    ## old version deprecated tt_tree = tt_tfile.Get('tree')
    ## old version deprecated tt_tree.AddFriend('sf/t', tt_ffile)
    ## old version deprecated em_tfile = ROOT.TFile('/afs/cern.ch/work/m/mdunser/public/edgeTrees/MuEG_all/MuEG/treeProducerSusyEdge/tree.root')
    ## old version deprecated #em_ffile = ROOT.TFile('/afs/cern.ch/work/m/mdunser/public/edgeTrees/MuEG_all/friends/evVarFriend_MuEG.root')
    ## old version deprecated em_ffile = ROOT.TFile('/afs/cern.ch/work/s/sesanche/public/nTuples/mar02/evVarFriend_MuEG.root')
    ## old version deprecated em_tree = em_tfile.Get('tree')
    ## old version deprecated em_tree.AddFriend('sf/t', em_ffile)
    ## new version with only friends
    tt_tfile = ROOT.TFile('/afs/cern.ch/work/m/mdunser/public/edgeTrees/friendsForWorkshop/new/evVarFriend_TTLep_pow.root')
    tt_tree = tt_tfile.Get('sf/t')
    em_tfile = ROOT.TFile('/afs/cern.ch/work/m/mdunser/public/edgeTrees/friendsForWorkshop/new/evVarFriend_MuEG.root')
    ...
    dss.append(em_data)
    if opts.dott: 
        #tt_mc   = pdfClass('tt_mc'   , tt_tree, opts.skimVal, 87314.8*2.3/5.19e7)
        tt_mc   = pdfClass('tt_mc'   , tt_tree, opts.skimVal, 87314.8*2.3/5.00e6)
        dss.append(tt_mc)
    ...
    writeobjs = []
    for var in ['ldp', 'a3d', 'met', 'mlb', 'ldr', 'zpt']:
    ...
    w.writeToFile("pdfs/pdfs_version24_savingTheWorkspace_june2016.root")

    outfile = ROOT.TFile('pdfs/pdfs_version24_extendedFit_june2016.root', 'RECREATE')
    outfile.cd()
    for i in writeobjs:
        i.Write()
    for i in frs:
