...
import math, sys, optparse, copy, re
...
def makeConciseTable(binnedSR0b, binnedSR1b):
    ret = ''
    for eta in ['central', 'forward']:
        ...
        for i in bin:
            mr = 'lm' if i == 0 else 'bz' if i == 1 else 'oz' if i == 2 else 'az' if i == 3 else 'hm'
            tmp_rinout   = getattr(ingDA, 'rinout_dy_%s.%s_val'%(mr, _eta ))
            tmp_rinout_e = getattr(ingDA, 'rinout_dy_%s.%s_err'%(mr, _eta ))
            onz_0b   = tmp_rinout*onZ.cen0b
            onz_1b   = tmp_rinout*onZ.cen1b
            onz_0b_e = math.sqrt(getattr(onZ, '%s0b_e'%_eta)**2 + (tmp_rinout_e*getattr(onZ, '%s0b'%_eta))**2 )
            onz_1b_e = math.sqrt(getattr(onZ, '%s1b_e'%_eta)**2 + (tmp_rinout_e*getattr(onZ, '%s1b'%_eta))**2 )

            ret += '&  %6.2f $\\pm$ %6.2f  &  \\multirow{2}{*}{\\textbf{ %3d }} &   %6.2f $\\pm$ %6.2f  &  \\multirow{2}{*}{\\textbf{ %d }} \\ \r'
            ret += '& (%6.2f $\\pm$ %6.2f) &                             %3s    &  (%6.2f $\\pm$ %6.2f) &                                   \\ \\cline{2-6} \r'%(
            predhisto0b.GetBinContent(i), predhisto0b.GetBinError(i), obshisto0b.GetBinContent(i), 
            predhisto1b.GetBinContent(i), predhisto1b.GetBinError(i), obshisto1b.GetBinContent(i), 
            onz0b, onz0b_e ,onz1b, onz1b_e )
...
def makePlot(pHisto, pHistoName, oHisto, oHistoName, plotname, eta, nbs):
    ...
    pHisto.GetYaxis().SetRangeUser(0., 1.8*maxCont)

    plot = Canvas.Canvas('results/%s/plot_results_mll_%s_%s%s_nb%d'%(lumi_str, plotname, eta, ('' if not opts.onlyTTbar else '_onlyTT'), nbs), 'png,pdf', 0.6, 0.65, 0.85, 0.82)
...
if __name__ == '__main__':
    ...
    nbcut_1 = 't.nBJetMedium35_Edge >= 0'
    ...
    tables = []
    for signalRegion in [signalRegion0b, signalRegion1b]:
        nb = 2 if '2b' in signalRegion.name else 1 if '1b' in signalRegion.name else '0b'
        nbstring = 'n_{b} '+('#geq ' if True else '= ')+str(nb)
        for eta in ['central', 'forward']:
            ...
            makePlot(mcPredHisto, 'pred. (MC)'  , mcObsHisto , 'obs. (MC)'   , 'MCClosure'    , eta, nb) ## closure plot
            makePlot(mcPredHisto, 'pred. (MC)'  , daPredHisto, 'pred. (DATA)', 'dataMCPred'   , eta, nb) ## prediction comparison
            makePlot(daPredHisto, 'pred. (DATA)', mcObsHisto , 'obs. (MC)'   , 'predDataObsMC', eta, nb) ## predData, obsMC
            makePlot(mcObsHisto , 'obs. (MC)'   , daObsHisto , 'obs. (DATA)' , 'dataMCobs'    , eta, nb) ## observed data/mc
            makePlot(daPredHisto, 'pred. (DATA)', daObsHisto , 'obs. (DATA)' , 'data'         , eta, nb) ## money plot
