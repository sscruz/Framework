...
if __name__ == "__main__":
    ...
    exs = []
    eys = []
    f = open("datacards/neuneuXsec.txt")
    xsecs = eval(f.read())
    ...
    rexp2p = []
    robs = []
    rfile = r.TFile("limits_NeuNeu_Moriond2017.root")
    ...
    label = "m(#tilde{#chi}^{0}_{2}) = m(#tilde{#chi}^{0}_{1}) [GeV]"
    ...
    aux.GetYaxis().SetRangeUser(10, 300000)
    
    plot = Canvas.Canvas('TChiZH', 'png,pdf,root', 0.45, 0.5, 0.65, 0.7)
    ...
    #plot.redrawAxis()
    plot.addLatex(0.2, 0.85, "pp #rightarrow #tilde{#chi}^{0}_{1} #tilde{#chi}^{0}_{2}; m(#tilde{#chi}^{0}_{1}) = m(#tilde{#chi}^{0}_{2})")
    plot.addLatex(0.2, 0.78, "#tilde{#chi}^{0}_{2}#rightarrow Z + #tilde{G}, #tilde{#chi}^{0}_{1}#rightarrow H + #tilde{G}")
